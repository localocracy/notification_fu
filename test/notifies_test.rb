...
class NotifiesTest < Test::Unit::TestCase
  def setup
  ...
  def test_should_fire_the_appropriate_callback
    ...
    Notification.expects(:create!).with(:actor => @james, :subject => @list, :event_type => 'list_created_or_updated')
    @list.save
    Notification.expects(:create!).with(:actor => @mat, :subject => @list, :event_type => 'list_created_or_updated')
    @list.author = @mat
    @list.save
  end
  ...
  def test_should_fire_event_with_secondary_subject
    @list = List.new(hash_for_list(:author => @james));
    Notification.stubs(:create!)
    @list.save
    @comment = Comment.new(:body => 'cool list!', :author => @mat, :list => @list)
    Notification.expects(:create!).with(:actor             => @mat, 
                                         :subject           => @comment, 
  ...
  def test_should_only_fire_if_the_condition_evaluates_to_true
    Notification.expects(:create!).with(:actor => @mat, :subject => @james, :event_type => 'follow_created')
  ...
  def test_should_fire_event_with_symbol_based_if_condition_that_is_true
    @james.fire = true
    Notification.expects(:create!).with(:subject => @james, :event_type => 'person_updated')
    @james.save
  end
  ...
  def test_should_set_secondary_subject_to_self_when_requested
    @list = List.new(hash_for_list(:author => @james));
    Notification.stubs(:create!).with(has_entry(:event_type, "list_created_or_updated"))
    @list.save
    @comment = Comment.new(:body => 'cool list!', :author => @mat, :list => @list)
    Notification.stubs(:create!).with(has_entry(:event_type, "comment_created"))
    @comment.save
    Notification.expects(:create!).with(:actor             => @mat, 
                                         :subject           => @list, 
