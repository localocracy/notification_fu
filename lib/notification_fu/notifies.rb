    ...
    module ClassMethods
      def notifies(event_type, opts)
        ...
        if opts[:on].kind_of?(Array)
          opts[:on].each { |on| notifies(event_type, opts.merge({:on => on})) }
          return
        end

        opts[:subject] = :self unless opts.has_key?(:subject)

        method_name = :"notify_#{event_type}_after_#{opts[:on]}"
        define_method(method_name) do
          ...
          Notification.create!(create_options)
        end
